...
static void nfs_program_2(struct svc_req *rqstp, SVCXPRT *transp);
...
nfs_program_2(struct svc_req *rqstp, SVCXPRT *transp)
{
	...
	case NFSPROC_GETATTR:
		xdr_argument = (xdrproc_t) xdr_fhandle;
...
main( int argc, char* argv[] )
{
	...
	if ((_rpcfdtype == 0) || (_rpcfdtype == SOCK_DGRAM)) {
		...
		if (!_rpcpmstart)
			proto = IPPROTO_UDP;
		if (!svc_register(transp, NFS_PROGRAM, NFS_VERSION, nfs_program_2, proto)) {
	...
	if ((_rpcfdtype == 0) || (_rpcfdtype == SOCK_STREAM)) {
		...
		if (!_rpcpmstart)
			proto = IPPROTO_TCP;
		if (!svc_register(transp, NFS_PROGRAM, NFS_VERSION, nfs_program_2, proto)) {
			_msgout("unable to register (NFS_PROGRAM, NFS_VERSION, tcp).");
			exit(1);
		}
	}
